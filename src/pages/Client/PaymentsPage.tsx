...
import { BanknotesIcon, CheckCircleIcon, ClockIcon, ExclamationTriangleIcon, ArrowDownTrayIcon, MagnifyingGlassIcon, BuildingOfficeIcon, ArrowRightIcon, ChartBarIcon, ShieldCheckIcon, CalendarDaysIcon } from "@heroicons/react/24/outline";
...
export const PaymentsPage: React.FC = () => {
  ...
  const onPaymentSuccess = async () => {
    ...
    try {
      setIsProcessingPayment(selectedPayment.id);
      await clientAPI.makePayment(selectedPayment.id, selectedPayment.amount, "Stripe");

      // Update local state
      setPayments((prev) => prev.map((p) => (p.id === selectedPayment.id ? { ...p, status: "paid" as const, paidDate: new Date() } : p)));
  ...
  const filteredPayments = payments.filter((payment) => {
    ...
    return payment.installmentNumber.toString().includes(searchLower) || payment.status.toLowerCase().includes(searchLower) || projectName.includes(searchLower) || location.includes(searchLower);
  });
    ...
    <div className="space-y-10 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        ...
        <div className="flex gap-4">
          <Button variant="outline" className="border-gold-500/20 text-gold-400 hover:bg-gold-500/10 rounded-xl px-6" onClick={() => handleExport("pdf")}>
                    ...
                    <div className="flex items-start justify-between">
                      <div className="space-y-4">
                        ...
                        <div>
                          ...
                          <h3 className="text-2xl font-bold text-white" style={{ color: stat.color }}>
                ...
                upcomingPayments.map((p) => (
                  <div key={p.id} className="group p-5 rounded-2xl bg-[#141414]/50 border border-gold-500/5 hover:border-gold-500/20 transition-all cursor-pointer">
                    <div className="flex justify-between items-start mb-4">
                      <div className="space-y-1">
                        <h4 className="text-white font-bold text-sm">
                          #{p.installmentNumber} • {p.project?.name || "Investment"}
                        </h4>
                      ...
                      <Badge className="bg-red-500/10 text-red-400 border-red-500/20 text-[10px] py-0.5">Action Required</Badge>
                    </div>
                    <div className="flex items-center justify-between pt-4 border-t border-gold-500/5">
                      ...
                      <Button size="sm" className="bg-gold-500 text-black hover:bg-gold-400 font-extrabold rounded-lg text-[10px] uppercase h-8" onClick={() => handleMakePayment(p)} disabled={isProcessingPayment === p.id}>
          ...
          <Card className="bg-[#0a0a0a]/60 border-gold-500/10 rounded-[2.5rem] backdrop-blur-md border shadow-2xl overflow-hidden">
            <CardHeader className="p-10 border-b border-gold-500/10 flex flex-col md:flex-row md:items-center justify-between gap-6">
              ...
              <div className="relative w-full md:w-64">
                ...
                <Input placeholder="Search Ledger..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="bg-black/40 border-gold-500/20 pl-10 h-11 rounded-xl text-sm focus:border-gold-500/50" />
              </div>
            </CardHeader>
                  ...
                  <TableBody>
                    <AnimatePresence>
                      {filteredPayments.map((payment, idx) => (
                        <motion.tr key={payment.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: idx * 0.05 }} className="group border-b border-gold-500/5 hover:bg-gold-500/[0.02] transition-colors">
                          ...
                          <TableCell>
                            <div className="space-y-1">
                              ...
                              {payment.paidDate && <p className="text-[10px] text-green-500 font-bold uppercase">Paid: {formatDate(payment.paidDate)}</p>}
                            </div>
                          </TableCell>
                          <TableCell>
                          ...
                          <TableCell className="pr-10 text-right">
                            {payment.status === "pending" || payment.status === "overdue" ? (
                              <Button size="sm" className="bg-gold-500 text-black hover:bg-gold-400 font-bold rounded-lg text-[10px] uppercase h-8" onClick={() => handleMakePayment(payment)} disabled={isProcessingPayment === payment.id}>
      ...
      <Dialog open={showStripeDialog} onOpenChange={setShowStripeDialog}>
        <DialogContent className="bg-[#0a0a0a] border-gold-500/20 text-white rounded-[2rem] p-10 max-w-xl">
          <DialogHeader>
            <DialogTitle className="text-3xl font-serif text-gold-500">Secure Payment</DialogTitle>
            <DialogDescription className="text-gray-500 text-sm">
              Paying for {selectedPayment?.project?.name || "Investment"} - Installment #{selectedPayment?.installmentNumber}
            </DialogDescription>
          </DialogHeader>
          {selectedPayment && (
            <div className="mt-8">
              <CheckoutForm amount={selectedPayment.amount} paymentId={selectedPayment.id} onSuccess={onPaymentSuccess} onCancel={() => setShowStripeDialog(false)} />
            </div>
          )}
        </DialogContent>
      </Dialog>
