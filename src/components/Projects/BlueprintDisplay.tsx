...
import { XMarkIcon, InformationCircleIcon, ShoppingCartIcon } from "@heroicons/react/24/outline";
...
export const BlueprintDisplay: React.FC<BlueprintDisplayProps> = ({ projectId, activeTab: controlledTab, showTabs = true }) => {
  ...
  const handleEnrollment = async () => {
    ...
    setIsEnrolling(true);
    try {
      // Parse price from string like "PKR 50 Lakh"
      let totalPrice = 0;
      const priceStr = selectedRoom.price || "";
      if (priceStr.includes("Lakh")) {
        totalPrice = parseFloat(priceStr.replace("PKR ", "").replace(" Lakh", "")) * 100000;
      } else if (priceStr.includes("Cr")) {
        totalPrice = parseFloat(priceStr.replace("PKR ", "").replace(" Cr", "")) * 10000000;
      } else {
        totalPrice = 5000000; // Default
      }
      ...
      await enrollmentAPI.createEnrollment({
        ...
        monthlyInstallment: (totalPrice * 0.8) / 36,
      });

      alert(`Successfully enrolled in Unit ${selectedRoom.id}! Redirecting to payments...`);
      navigate("/payments");
    } catch (error: any) {
  ...
  return (
    <div className="space-y-6">
        ...
        {loading ? (
          ...
          <div className="flex flex-col lg:flex-row gap-6">
            ...
            <div className="lg:w-1/2">
              ...
              >
                <img src={activeTab === "economy" ? "/blueprints/ECONOMY_FLATS.png" : activeTab === "premium" ? "/blueprints/PREMIUM_FLATS.png" : "/blueprints/PENTHOUSE.png"} alt={`${activeTab} Floor Plan`} className="w-full h-auto" style={{ background: "#ffffff", minHeight: "500px", objectFit: "contain" }} />
              </div>
            </div>
      ...
      <AnimatePresence>
        {selectedRoom && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4" onClick={() => setSelectedRoom(null)}>
            <motion.div
            ...
            >
              <div className="flex items-center justify-between mb-6">
                ...
                <button onClick={() => setSelectedRoom(null)} className="p-2 rounded-lg hover:bg-white/10 transition-colors">
              ...
              <div className="space-y-4">
                ...
                {selectedRoom.tenant && (
                  ...
                  >
                    <p className="text-xs mb-1" style={{ color: "rgba(156, 163, 175, 0.9)" }}>
                      Tenant
                    </p>
                ...
                >
                  ...
                  <p className="text-xs" style={{ color: "rgba(156, 163, 175, 0.9)" }}>
                    {selectedRoom.status === "vacant" ? "This unit is available for booking. Contact our sales team for more information." : selectedRoom.status === "occupied" ? "This unit has been sold and is currently occupied." : "This unit is currently under maintenance and will be available soon."}
                  </p>
                </div>
                ...
                {selectedRoom.status === "vacant" && (
                  ...
                  >
                    ...
                    {isEnrolling ? "Processing..." : "Purchase This Unit"}
                  </motion.button>
                )}
              </div>
            </motion.div>
