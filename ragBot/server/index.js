...
// In-memory chat memory (per process). For multi-tenant or prod, persist by user/session.
let conversationHistory = []; // Array<{ role: 'user' | 'assistant', content: string }>
let memorySummary = '';       // Summarized older context

// Memory config
const MAX_RECENT_MESSAGES = 15;   // inject last 10-20 messages
const MAX_HISTORY_LENGTH = 60;    // total messages before summarizing
const MAX_MEMORY_CHARS = 6000;    // rough token safety guard
const MAX_SUMMARY_CHARS = 1500;   // cap per-request summary

/**
 * Builds a string representation of the conversation history to send to the model.
 * Includes a summary of older turns (if any) plus the most recent messages.
 */
const buildHistoryContext = () => {
  const recent = conversationHistory.slice(-MAX_RECENT_MESSAGES);
  const historyLines = recent.map(msg => `${msg.role.toUpperCase()}: ${msg.content}`);

  let context = '';
  if (memorySummary) {
    context += `Earlier summary: ${memorySummary}\n`;
  }
...
/**
 * Summarizes older messages to keep context within budget.
 * Simple heuristic summary to avoid extra model calls.
 */
const summarizeHistory = () => {
  if (conversationHistory.length <= MAX_RECENT_MESSAGES) return;

  const older = conversationHistory.slice(
    0,
    Math.max(0, conversationHistory.length - MAX_RECENT_MESSAGES)
  );

  if (!older.length) return;

  // Lightweight summarization: concatenate and trim
  ...
  memorySummary = memorySummary
    ? `${memorySummary} | ${condensed}`
    : condensed;

  // Keep only the recent window
  conversationHistory = conversationHistory.slice(-MAX_RECENT_MESSAGES);
};

/**
 * Checks if memory exceeds limits and triggers summarization.
 */
const maintainMemoryBudget = () => {
  const charCount =
...
/**
 * Builds a concise summary string of the entire conversation for prompt injection.
 * Lightweight concatenation to avoid extra model calls; truncated for safety.
 */
const buildConversationSummary = () => {
  ...
  // Keep the tail to preserve most recent specifics while retaining earlier summary
...
app.post('/api/gemini/query', async (req, res) => {
  ...
  // Build conversation summary + recent turns for persistent memory
...
Conversation summary (use these facts as given by the user/assistant):
${conversationSummary}

Conversation history (use this to preserve context across turns):
${historyContext}
...
Now provide your response in plain text without any formatting:`;
    ...
    // Record assistant response and maintain budget
    conversationHistory.push({ role: 'assistant', content: processedText });
    // Refresh memory summary each turn so specific user-provided facts persist
    memorySummary = buildConversationSummary();
    maintainMemoryBudget();
